...
<div class="card-header my-3"><fmt:message key="locale.AllProducts" /></div>
    <div class="footer-copyright text-center py-3">
        <a href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=showGoodsList&sort=sortByName"
           class="btn btn-light"><fmt:message key="locale.SortByName" /></a>
        <a href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=showGoodsList&sort=sortByPrice"
           class="btn btn-light"><fmt:message key="locale.SortByPrice" /></a>
        <a href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=showGoodsList&sort=sortByNovelty"
           class="btn btn-light"><fmt:message key="locale.SortByNovelty" /></a>
        <a href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=showGoodsList&sort=default"
        ...
        <div class="card w-100">
            ...
            <div class="card-body">
                ...
                <div class="mt-3 d-flex justify-content-between">
                    <a class="btn btn-dark" href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=addToCard&goodsId=<c:out value='${goods.id}'/>"><fmt:message key="locale.AddToCard" /></a> <a
                        class="btn btn-primary" href="${pageContext.request.contextPath}/FrontController?command=CATALOG_COMMAND&action=buyNow&goodsId=<c:out value='${goods.id}'/>"><fmt:message key="locale.BuyNow" /></a>
                </div>
            </div>
        </div>
    </div>
